...
class PageController extends AbstractController
{
    ...
    public function store()
    {
        ...
        return Redirect::route('pages.show', ['pages' => $page->slug])
            ->with('success', 'Sua página foi criada com sucesso.');
    }
    ...
    public function update($slug)
    {
        ...
        // write flash message and redirect
        return Redirect::route('pages.show', ['pages' => $page->slug])
            ->with('success', 'Sua página foi atualizada com sucesso.');
    }
    ...
    public function destroy($slug)
    {
        ...
        try {
            ...
            return Redirect::route('pages.show', ['pages' => $page->slug])
                ->with('error', 'Você não pode apagar esta página.');
        }

        // write flash message and redirect
        return Redirect::to('pages/home')->with('success', 'Sua página foi apagada com sucesso.');
    }
    ...
    protected function checkPage($page, $slug)
    {
        ...
        if ($slug == 'home') {
            throw new Exception('Está faltando a homepage.');
        }
        ...
        if ($slug == 'home') {
            if ($slug != $input['slug']) {
                return Redirect::route('pages.edit', ['pages' => $slug])->withInput()
                    ->with('error', 'Você não pode alterar o slug desta página.');
            }

            if ($input['show_nav'] == false) {
                return Redirect::route('pages.edit', ['pages' => $slug])->withInput()
                    ->with('error', 'A página inicial deve permanecer na barra de navegação.');
            }
        }
    }
}
